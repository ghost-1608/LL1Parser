/* 
 * An LL1 parser written in C. I re-wrote the parser I wrote in Python in C. Makes use of a hash table in place of the Python Dict used in the original code.
 * 
 * The only drawback is that the hash table isn't a true hash table. It functions by storing the keys and values in two separate arrays, using the same index.
 * 
 * (ghost-1608 26/11/23)
 */
...
typedef enum
{ HTBL_ERR = -1, HTBL_NULL, HTABLE_t, STR_t } HTBL_ENM;
...
int ll1 (char *inp_arg, char start_symbol, HTABLE *table)
{
  ...
  strncpy (inp, inp_arg, 64);   // Copying to safely modify input string buffer
  strcat (inp, "$");
...
{
  if (cstk->top == cstk->size or cstk->top < 0)
    return -1;
